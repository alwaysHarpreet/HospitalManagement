...
// Middleware to verify JWT token for lab technician role
export const isLabTechAuthenticated = asyncHandler(async (req, res, next) => {
    const token = req.cookies.labTechToken;
    ...
    const decoded = jwt.verify(token, process.env.JWT_SECRET_KEY);
    req.labTech = await LabTechnician.findById(decoded.id);
    if (!req.labTech || req.labTech.role !== "LabTechnician") {
